...
2. Watch the scraper run (Firecrawl → Claude consolidation → generic `knowledge.md`).
...
| ---- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| W2.1 | Scraper pipeline in `apps/backend/scraper/`: `firecrawlMap(url)` → `firecrawlCrawl(urls)` → `consolidate(markdown[], verticalConfig?)` (Claude Sonnet 4.6, structured Zod-validated output). Hard rule in prompt: "do not invent prices; mark `unknown` if not in source." Writes `data/clinics/<tenant_id>/knowledge.md` + structured JSON to Supabase.                  | W1.1, W1.2, W1.3 | Run end-to-end against one real Polish business site (any vertical — pick a vet clinic for now since vet is the leading candidate). Output `knowledge.md` has Klinika/Tenant + Staff + Services + FAQ sections. No invented prices (manual spot-check). Structured JSON validates against `ScraperOutputSchema`. |
...
| W2.5 | Universal consent flow in `apps/backend/consent/`: Polish + English script (vertical-independent), classifier (Claude Haiku 4.5, Zod-validated output, ambiguous-defaults-false). Exported as both a system-prompt fragment and a post-turn classifier.                                                                                                                   | W1.2             | Vitest test: 10 transcripts (5 "tak", 5 "nie", 2 ambiguous). Classifier returns yes/no correctly on 8/10; ambiguous defaults to no. PII-redaction test: log capture contains zero phone numbers / names.                                                                                                         |
...
| ElevenLabs Anthropic Claude Sonnet 4.6 LLM not exposed in ConvAI yet | Fall back to GPT-4o (also EU residency). Document in AI-SPEC Section 2 as a known unknown to verify Day 8 morning.                   |
...
| Claude/OpenAI rate limits during scraper consolidation               | Add p-retry with exponential backoff; consolidate per-chunk in parallel with concurrency cap = 3.                                    |
| ElevenLabs Grants not yet approved                                   | Continue on Creator $22 → Pro $99 from W2. Trivial cost.                                                                             |
...
| Before W2.1 consolidation step runs      | Anthropic + OpenAI API keys in `.env.local`                                                                                                                                       | Yes                          |
| Before W2.2 provisions a real agent      | ElevenLabs Creator workspace + API key in `.env.local`; workspace setting "Use conversation data for model improvement" toggled OFF in their UI by you (I cannot do this via API) | Yes                          |
