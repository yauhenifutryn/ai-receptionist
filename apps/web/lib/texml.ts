...
/**
 * Played when a clinic's demo agent has exhausted its per-clinic minute
 * budget (silent backstop — see apps/web/lib/demo-budget.ts). A correct PIN
 * still routes here instead of <Dial> so one clinic cannot drain the shared
 * ElevenLabs credit pool and starve the others. Brand-voice prompt, same as
 * the other IVR mp3s (generated by scripts/generate-ivr-prompts.mjs).
 */
export function limitReachedTexml(baseUrl: string): string {
  if (/["&<>]/.test(baseUrl)) throw new Error("baseUrl contains XML-unsafe characters");
  return [
    XML_HEADER,
    `<Response>`,
    `<Play>${baseUrl}/ivr/limit-reached.mp3</Play>`,
    `<Hangup/>`,
    `</Response>`,
  ].join("");
}
