...
describe("handleCheckAvailability (W2.3)", () => {
  it("returns exactly 3 slots with future start times, Zod-valid", () => {
    const out = handleCheckAvailability(
      {
  ...
  it("slotIds round-trip through the codec", () => {
    const out = handleCheckAvailability(
      {
  ...
  it("displayLabel is localized by language", () => {
    const polish = handleCheckAvailability(
      {
    ...
    const english = handleCheckAvailability(
      {
  ...
  it("throws when request body is invalid", () => {
    expect(() =>
      handleCheckAvailability({ wrong: "shape" }, { now: () => FROZEN_NOW }),
    ).toThrow();
  });

  it("duration is category-dependent (complex_service longer than information_only)", () => {
    const complex = handleCheckAvailability(
      {
    ...
    const info = handleCheckAvailability(
      {
